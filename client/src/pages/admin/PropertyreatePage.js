import { Button, Form, Input, List, message, Tabs, Typography } from "antd";
    ...
    try {
      if (editItems[type]) {
        await handleSaveEdit(
          type,
          type === "status" || type === "env" || type === "condition" ? "name" : type,
      ...
      <Tabs activeKey={activeTab} onChange={setActiveTab}>
        ...
        <TabPane tab="Особые пометки" key="note">
          <Form
            form={forms.note}
            onFinish={(values) =>
              handleSubmit(values, "note", createSpecialNote, fetchSpecialNotes)
            }
            layout="vertical"
          >
            <Form.Item
